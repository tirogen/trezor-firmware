...
impl<'a> LockScreen<'a> {
    ...
    //     display::avatar(screen().center(), self.avatar, theme::WHITE,
    // theme::BLACK);     display::text_center(Point::new(screen().center().x,
    // 35),self.device_name,theme::FONT_BOLD, theme::GREY_LIGHT, theme::BG); }

    pub fn paint_locked(&self) {
        display::text_center(
            Point::new(screen().center().x, 35),
            self.device_name,
            Font::BOLD,
            theme::GREY_LIGHT,
            theme::BG,
        ...
        if let Some(label) = self.lock_label {
            ...
            display::rect_fill_rounded(bar_area, theme::GREY_LIGHT, theme::BG, 4);

            display::rect_fill_rounded(bar_area_in, theme::BG, theme::GREY_LIGHT, 4);

            display::text_center(
                Point::new(screen().center().x, 128),
                label,
                Font::BOLD,
                theme::GREY_LIGHT,
                theme::BG,
        ...
        if let Some(label) = self.tap_label {
            // # "tap to unlock"
            display::text_center(
                Point::new(screen().center().x + 10, 220),
                label,
                Font::BOLD,
                theme::GREY_LIGHT,
                theme::BG,
