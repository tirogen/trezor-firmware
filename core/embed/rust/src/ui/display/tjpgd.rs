...
fn reinterpret_slice<T>(slice: &[i32]) -> &[T] {
    let len = slice.len() * mem::size_of_val(&slice[0]) / mem::size_of::<T>();
    let ptr = slice.as_ptr() as _;
...
fn reinterpret_slice_mut<T>(slice: &mut [i32]) -> &mut [T] {
    let len = slice.len() * mem::size_of_val(&slice[0]) / mem::size_of::<T>();
...
impl<'i, 'p> JDEC<'i, 'p> {
            ...
            if cmp != 0 && self.ncomp as i32 != 3 {
                ...
                if JD_FORMAT != 2 || cmp == 0 {
                    ...
                    if z == 1 || JD_USE_SCALE != 0 && self.scale == 3 {
                        ...
                        Self::block_idct(&mut self.workbuf, &mut self.mcubuf[mcu_buf_idx..]);
                    }
                }
            }
    ...
    fn mcu_output(
        ...
        let workbuf: &mut [u8] = reinterpret_slice_mut(self.workbuf);
        ...
        let bitmap: &[u16] = reinterpret_slice(self.workbuf);
...
pub fn jpeg<'i>(data: &'i [u8], pos: Point, scale: u8) {
...
pub fn jpeg_info<'i>(data: &'i [u8]) -> Option<(Offset, i16)> {
    let pool = unsafe { get_jpeg_work_buffer(0, true).buffer.as_mut_slice() };
    let mut inp = BufferInput(data);
...
pub fn jpeg_test<'i>(data: &'i [u8]) -> bool {
    let pool = unsafe { get_jpeg_work_buffer(0, true).buffer.as_mut_slice() };
    let mut inp = BufferInput(data);
    if let Ok(mut jd) = JDEC::new(&mut inp, pool) {
        if jd.mcu_height() > 16 {
...
impl<'i> JpegInput for BufferInput<'i> {
    fn read(&mut self, inbuf: Option<&mut [u8]>, n_data: usize) -> usize {
        ...
        if let Some(inbuf) = inbuf {
            (&mut inbuf[..len]).copy_from_slice(toread)
        }
        self.0 = newdata;
        n_data
    }
