use crate::ui::{
    ...
    display::{icon, rect_fill, rect_fill_rounded, rect_fill_rounded1, Color},
...
pub fn loader_starry_indeterminate(
    ...
    if let Some((data, color)) = icon_ {
        icon(area.center(), data, color, bg_color);
    }
}
