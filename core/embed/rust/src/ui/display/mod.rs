...
pub use crate::ui::display::toif::{Icon, Image};
#[cfg(any(feature = "model_tt", feature = "model_tr"))]
...
pub fn rect_rounded2_partial(
    ...
    if let Some((icon, icon_color)) = icon {
        if icon.width() <= MAX_ICON_SIZE && icon.height() <= MAX_ICON_SIZE {
            icon_area = Rect::from_center_and_size(center, icon.size());
            icon_area_clamped = icon_area.clamp(constant::screen());
            let mut ctx = UzlibContext::new(icon.data(), None);
...
pub fn text_over_image(
    ...
    if let Some((a, color)) = bg_area {
        ...
        area = a;
        r_img = Rect::from_top_left_and_size(a.top_left() + offset_img, image.size());
        offset_img_final = offset_img;
    } else {
        area = Rect::from_top_left_and_size(offset_img.into(), image.size());
        r_img = area;
    ...
    let mut ctx = UzlibContext::new(image.data(), Some(&mut window));
...
pub fn icon_over_icon(
    ...
    assert!(icon_bg.width() <= constant::WIDTH);
    assert_eq!(icon_bg.width() % 2, 0);

    assert!(icon_fg.width() <= constant::WIDTH);
    assert_eq!(icon_fg.width() % 2, 0);
    ...
    if let Some(a) = bg_area {
        area = a;
        r_bg = Rect::from_top_left_and_size(a.top_left() + offset_bg, icon_bg.size());
        final_offset_bg = offset_bg;
    } else {
        r_bg = Rect::from_top_left_and_size(Point::new(offset_bg.x, offset_bg.y), icon_bg.size());
        area = r_bg;
        final_offset_bg = Offset::zero();
    }

    let r_fg = Rect::from_top_left_and_size(area.top_left() + offset_fg, icon_fg.size());
    ...
    let mut ctx_bg = UzlibContext::new(icon_bg.data(), Some(&mut window_bg));

    let mut window_fg = [0; UZLIB_WINDOW_SIZE];
    let mut ctx_fg = UzlibContext::new(icon_fg.data(), Some(&mut window_fg));
