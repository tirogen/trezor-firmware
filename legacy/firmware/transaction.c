...
static const uint32_t SLIP19_VERSION_MAGIC = 0x19004c53;
...
bool get_ownership_proof(const CoinInfo *coin, InputScriptType script_type,
  ...
  if (script_type == InputScriptType_SPENDWITNESS) {
    ...
    r += ser_length(0, out->ownership_proof.bytes + r);  // Empty scriptSig.
    ...
    if (!tx_sign_bip340(node->private_key, digest, out->signature.bytes,
                        &out->signature.size)) {
      return false;
    }
    r += ser_length(0, out->ownership_proof.bytes + r);  // Empty scriptSig.
